...
document.querySelector(".nav-tabs").addEventListener("click", function(e) {
    ...
    let alltemp = document.querySelector(`.${state.section}`).querySelectorAll(".rl");
    alltemp.forEach((tem)=>tem.classList.remove("hiddenClass")
    );
    if (e.target.classList.contains("tab-all")) alltemp.forEach((temp)=>{
        if (temp.classList.contains("hiddenClass")) return temp.classList.remove("hiddenClass");
    });
    if (e.target.classList.contains("tab-resumes")) alltemp.forEach((tem)=>{
        if (!tem.classList.contains("resume")) return tem.classList.add("hiddenClass");
    });
    if (e.target.classList.contains("tab-cl")) alltemp.forEach((tem)=>{
        if (!tem.classList.contains("letter")) return tem.classList.add("hiddenClass");
    });
});
...
document.querySelector(".site-menu").addEventListener("click", function(e) {
    ...
    if (!buttonId) return;
    document.querySelector(".nav-tabs").querySelectorAll("a").forEach((elem)=>elem.classList.remove("active1")
    );
    headerEl.forEach((head)=>head.classList.remove("nav-open")
    );
...
const getCvOrLetter = async function(data) {
    ...
    if (state.user.file) {
        ...
        state.templateToUse.type === "resume" ? state.user.inputData.images = res.data : state.user.coverLetter.images = res.data;
    }
...
const createPdfMarckup = function(data) {
    ...
    if (data.template.type === "letter") {
        if (data.template.template === "letter1") marckup = `
  ...
  `;
        if (data.template.template === "letter2") marckup = `
  ...
  <div class="large-content">
    <div class="letterContainer" >
      ...
      <div contenteditable="false" id="letter">
        ...
        if (data.template.template === "letter3") marckup = `<div class="template rl template3 letter cover cover3" id="${data._id}">
  ...
  `;
        if (data.template.template === "letter4") marckup = `<div class="template rl template4 letter cover cover4" id="${data._id}">
  ...
  `;
        if (data.template.template === "letter5") marckup = `<div class="template rl template5 letter cover cover5" id="${data._id}">
  ...
  `;
        if (data.template.template === "letter6") marckup = `
  ...
  `;
    ...
    if (data.template.type === "resume") {
        if (data.template.template === "resume1") marckup = `<div id="${data._id}" class="template resume rl template1">
      <div class="user-name-and-profession">
      <h1 class="user-name">${data.fullName}</h1>
         <p class="profession">${data.profession}</p>
       </div>
  ...
  `;
        ...
        `;
        if (data.template.template === "resume2") marckup = `  <div id="${data._id}" class="template resume rl template2">
  ...
  <div class="certification informationContainer">
      ...
      ` : ""}
        ...
        if (data.template.template === "resume3") marckup = ` <div id="${data._id}" class="template resume rl template3"> 
  ...
  ` : ""}
      ...
      ` : ""}
        ...
        if (data.template.template === "resume4") marckup = ` <div id="${data._id}" class="template resume rl template4">
  ...
  `;
        if (data.template.template === "resume5") marckup = `
  ...
  `;
        if (data.template.template === "resume6") marckup = `   <div id="${data._id}" class="template resume rl template6">
...
allTemplates.forEach((tmf)=>{
    tmf.addEventListener("mouseleave", function(e) {
        let btnUseThis = e.target.closest(".resumeAndLetter").querySelector(".custom-btn");
...
myTemplates.addEventListener("click", function(e) {
    ...
    if (e.target.closest(".resumeAndLetter")) {
        ...
        let templateBtn = e.target.closest(".resumeAndLetter").querySelector("button");
        ...
        templateBtn.addEventListener("click", function(e) {
            ...
            state.templateToUse.template = this.closest(".resumeAndLetter").classList[2];
            // console.log(state.templateToUse);
            window.scrollTo({
                top: 0,
                behavior: "smooth"
            });
            if (state.templateToUse.type === "resume") return cvFormContainer.classList.add("showIt");
            if (state.templateToUse.type === "letter") return document.querySelector(".cover-letter-container").classList.add("showIt");
        });
    }
});
    ...
    imageIn.addEventListener("change", async function(e) {
        ...
        try {
            ...
            reader.onloadend = function() {
                document.querySelectorAll(".display_image2").forEach((display)=>display.style.backgroundImage = `url(${reader.result})`
                );
            };
